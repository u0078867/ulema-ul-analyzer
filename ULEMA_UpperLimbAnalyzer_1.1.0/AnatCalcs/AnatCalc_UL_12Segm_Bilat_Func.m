...
if ~isempty(i1)
    iMHA = strcmp({BODY.SEGMENT(i1).FunctionalAxis.Name},'RMHA');
    MHA = BODY.SEGMENT(i1).FunctionalAxis(iMHA).Marker1.Kinematics - BODY.SEGMENT(i1).FunctionalAxis(iMHA).Marker2.Kinematics;
    BODY.SEGMENT(i1).AnatomicalFrame.KinematicsPose=AnatomicalFrameDefinition_VECT('Right_Humerus_f',RGHr, REL, REM, MHA);
end
if ~isempty(i2)
...
if ~isempty(i7)
    ...
    MHA = BODY.SEGMENT(i1).FunctionalAxis(iMHA).Marker1.Kinematics - BODY.SEGMENT(i1).FunctionalAxis(iMHA).Marker2.Kinematics;
    BODY.SEGMENT(i7).AnatomicalFrame.KinematicsPose = AnatomicalFrameDefinition_VECT('Left_Humerus_f', LGHr, LEL, LEM, MHA);
end
if ~isempty(i8)
