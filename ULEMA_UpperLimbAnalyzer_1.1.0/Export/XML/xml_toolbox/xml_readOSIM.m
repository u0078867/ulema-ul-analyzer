function [tree, RootName, DOMnode] = xml_readOSIM(xmlfile, Pref)
...
DPref.NumLevels = 1e10;   % 
RootOnly        = true;   % return root node only
...
if (level>Pref.NumLevels), return; end
...
if (node.hasAttributes && Pref.ReadAttr)
  ...
  for iAttr = 1:Attr.getLength    % for each attribute
    ...
    value = str2var(value);       % convert to number if possible
...
str = regexprep(str,'function','xFunction'  ,'once', 'preservecase');
if (~isvarname(str))
  str = genvarname(str);  
end
