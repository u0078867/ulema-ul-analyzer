...
function GraphSRE
% GRAPHSRE [ BodyMech 3.06.01 ]: Plots SRE
% INPUT
         ...
         if iMuscle>=1 & iMuscle <=Nmuscles,
            if ~isempty(BODY.MUSCLE(iMuscle).Name),
               if ~isempty(BODY.MUSCLE(iMuscle).Emg.Envelope),
                  ...
                  Nsamples=length(BODY.MUSCLE(iMuscle).Emg.Envelope);
                  
                  iAxis=iAxis+1;
                  subplot(NselectedMuscles,1,iAxis)
                  
                  Sfreq=1/BODY.MUSCLE(iMuscle).Emg.EnvelopeTimeGain;
                  time=[1/Sfreq:1/Sfreq:Nsamples/Sfreq];
                  
                  plot(time,BODY.MUSCLE(iMuscle).Emg.Envelope,'b');
                  
                  % title:
                  Axranges=axis;
                  xmin=Axranges(1); xmax=Axranges(2); 
                  ymin=Axranges(3); ymax=Axranges(4); 
                  axis([xmin xmax 0 ymax]); % SRE is always positive
