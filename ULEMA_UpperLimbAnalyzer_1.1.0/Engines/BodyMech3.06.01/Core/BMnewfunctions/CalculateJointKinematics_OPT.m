...
function CalculateJointKinematics_OPT(method)
...
%                 cat(3,BODY.JOINT(iJoint).AnatomyRefKinematics.Pose, T); % to the 3rd (time-)dimension
            ...
            jointrot = BODY.JOINT(iJoint).AnatomyRefKinematics.Pose(1:3,1:3,iSample);
            [a b]=RotationMatrixToCardanicAngles(jointrot,DecompositionFormat);
            x(:,iSample)=a';
            y(:,iSample)=b';
        end
...
%                 cat(3,BODY.JOINT(iJoint).PostureRefKinematics.Pose, T); % to the 3rd (time-)dimension
            ...
            %  calculate joint-angles
            jointrot = BODY.JOINT(iJoint).PostureRefKinematics.Pose(1:3,1:3,iSample);
            [a b]=RotationMatrixToCardanicAngles(jointrot,DecompositionFormat);
            x(:,iSample)=a';
            y(:,iSample)=b';
        end
