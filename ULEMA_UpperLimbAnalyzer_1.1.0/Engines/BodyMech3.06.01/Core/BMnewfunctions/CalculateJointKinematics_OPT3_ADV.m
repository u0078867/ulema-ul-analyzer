...
function CalculateJointKinematics_OPT3_ADV(method, iJointsAbs, G_T_LAB)
...
%   iJointAbs: indices of the joints for which to perform the reference
%   based calculations, if method is 'ReferenceBased'
%   G_T_LAB: matrix from lab reference frame to global reference frame
% PROCESS
...
AngleDecompositionSolution='FirstSolution'; %default
...
if strcmp(method,'AnatomyBased'),
    ...
    for iJoint=1:Njoints,  % for each joint
        ...
        if isfield(BODY.JOINT(iJoint).AnatomyRefKinematics,'AngleSolution');
            AngleDecompositionSolution=BODY.JOINT(iJoint).AnatomyRefKinematics.AngleSolution;
        else
            AngleDecompositionSolution='FirstSolution';
        end
        Nsamples=size(BODY.SEGMENT(DistSegm).Cluster.KinematicsPose,3);
        BODY.JOINT(iJoint).AnatomyRefKinematics.Pose = zeros(4,4,Nsamples);
        for iSample=1:Nsamples,  % for each time-instance 
            
            if ProxSegm==0,
                T=G_T_LAB * BODY.SEGMENT(DistSegm).AnatomicalFrame.KinematicsPose(:,:,iSample);                
...
%                 cat(3,BODY.JOINT(iJoint).AnatomyRefKinematics.Pose, T); % to the 3rd (time-)dimension
        ...
        [a b]=RotationMatrixToCardanicAngles_VECT(jointrot,DecompositionFormat);
        x = a';
        y = b';
        
        if strcmp(AngleDecompositionSolution,'FirstSolution'),
            BODY.JOINT(iJoint).AnatomyRefKinematics.RotationAngles=180/pi*x;
        else
            BODY.JOINT(iJoint).AnatomyRefKinematics.RotationAngles=180/pi*y;
        end
...
elseif strcmp(method,'ReferenceBased')
    ...
    for iJoint = iJointsAbs
        ...
        if isfield(BODY.JOINT(iJoint).PostureRefKinematics,'AngleSolution');
            AngleDecompositionSolution=BODY.JOINT(iJoint).PostureRefKinematics.AngleSolution;
        else
            AngleDecompositionSolution='FirstSolution';
        end
        Nsamples=size(BODY.SEGMENT(DistSegm).Cluster.PostureRefKinematicsPose,3);
        BODY.JOINT(iJoint).PostureRefKinematics.Pose = zeros(4,4,Nsamples);
        for iSample=1:Nsamples,  % for each time-instance 
...
%                 cat(3,BODY.JOINT(iJoint).PostureRefKinematics.Pose, T); % to the 3rd (time-)dimension
        ...
        if strcmp(AngleDecompositionSolution,'FirstSolution'),
            ...
            BODY.JOINT(iJoint).PostureRefKinematics.RotationAngles=180/pi*y;
        end
