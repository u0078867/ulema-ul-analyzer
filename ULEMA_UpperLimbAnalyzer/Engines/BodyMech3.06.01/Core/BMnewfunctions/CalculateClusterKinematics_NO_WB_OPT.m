...
function CalculateClusterKinematics_NO_WB_OPT(segment_range)
...
for r = segment_range
    ...
    if ~isempty(BODY.SEGMENT(segment_no).Cluster.MarkerCoordinates)
        ...
        LocalCoordinates=zeros(Z-X+1, 5000);
        GlobalCoordinates=zeros(Z-X+1, 5000);
        for i=1:n_samples,
...
%                 T );                                                       % to the 3rd (time-)dimension
            ...
            BODY.SEGMENT(segment_no).Cluster.KinematicsPose(:,:,i) = T;
