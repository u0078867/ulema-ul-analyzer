...
for j = 1 : length(sessions)
    ...
    for k = 1: length(sessions(j).trials)
        ...
        if ~neverProc
            ...
            if bestCyclesFound == 1
                if isfield(tempDB.subjects(subInd).sessions(sesInd), 'bestCycles')
                    keepBestCycles = keepBestCycles && isEqualDeep(tempDB.subjects(subInd).sessions(sesInd).bestCycles, bestCycles);
                    keepBestCycles = 0;
    ...
    if bestCyclesFound == 1 && keepBestCycles == 1
        data.sessions(j).bestCycles = bestCycles;
    else
        if bestCyclesFound == 1
            msg{1} = ['When loading subject ', name, ', session ', sessions(j).name];
            msg{2} = 'different data for best cycles was found.';
            msg{3} = 'What do you want to do?';
            choice = questdlg(char(msg),'Subject loading warning','Merge best cycles (suggested)','Delete best cycles','Merge best cycles (suggested)');
            switch choice
                case 'Merge best cycles (suggested)'
                    data.sessions(j).bestCycles = mergeBestCycles(bestCyclesCache);
                case 'Delete best cycles'
