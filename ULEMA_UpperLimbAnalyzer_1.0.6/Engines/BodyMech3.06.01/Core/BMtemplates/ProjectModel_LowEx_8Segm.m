...
% PROJECTMODEL_LOWEX_8SEGM [ BodyMech 3.06.01 ]: Template to create New ProjectModel 
% for 8 segments: feet, lower legs, upper legs, pelvis and trunk
% INPUT
%    Input : none
% PROCESS
...
CreateBodySegment('Left_Foot',4,[10 11 12]); % Segment 4
CreateBodySegment('Left_Shank',5,[13 14 15]); % Segment 5
CreateBodySegment('Left_Thigh',6,[16 17 18]); % Segment 6
CreateBodySegment('Pelvis',7,[19 20 21]); % Segment 7
CreateBodySegment('Trunk',8,[22 23 24]); % Segment 8
...
% Left foot (4)
% Fill in your anatomical landmarks of the foot between the ''
BODY.SEGMENT(4).AnatomicalLandmark(1).Name={'Calcaneus_left'}; 
BODY.SEGMENT(4).AnatomicalLandmark(2).Name={'MTP_I_left'}; % metatarsale I
BODY.SEGMENT(4).AnatomicalLandmark(3).Name={'MTP_V_left'}; % metatarsale V

% Left shank (5)
% Fill in your anatomical landmarks of the lower leg between the ''
BODY.SEGMENT(5).AnatomicalLandmark(1).Name={'Caput_Fibulae_left'}; 
BODY.SEGMENT(5).AnatomicalLandmark(2).Name={'Tuberositas_Tibia_left'}; 
BODY.SEGMENT(5).AnatomicalLandmark(3).Name={'Malleolus_Lateral_left'}; 
BODY.SEGMENT(5).AnatomicalLandmark(4).Name={'Malleolus_Medial_left'}; 

% Left thigh (6)
% Fill in your anatomical landmarks of the upper leg between the ''
BODY.SEGMENT(6).AnatomicalLandmark(1).Name={'Trochanter_Major_left'}; 
BODY.SEGMENT(6).AnatomicalLandmark(2).Name={'Femur_Condyle_Lateral_left'}; 
BODY.SEGMENT(6).AnatomicalLandmark(3).Name={'Femur_Condyle_Medial_left'};

% Pelvis (7)
% Fill in your anatomical landmarks of the pelvis between the ''
BODY.SEGMENT(7).AnatomicalLandmark(1).Name={'ASIS_Right'}; % anterior superior iliac spine right
BODY.SEGMENT(7).AnatomicalLandmark(2).Name={'ASIS_Left'}; % anterior superior iliac spine left
BODY.SEGMENT(7).AnatomicalLandmark(3).Name={'PSIS_Right'}; % posterior superior iliac spine right
BODY.SEGMENT(7).AnatomicalLandmark(4).Name={'PSIS_Left'}; % posterior superior iliac spine left

% Trunk (8)
% Fill in your anatomical landmarks of the trunk between the ''
BODY.SEGMENT(8).AnatomicalLandmark(1).Name={'IJ'}; % incisura jugularis
BODY.SEGMENT(8).AnatomicalLandmark(2).Name={'PX'}; % processus xiphoideus
BODY.SEGMENT(8).AnatomicalLandmark(3).Name={'C7'}; % processus spinosus C7
BODY.SEGMENT(8).AnatomicalLandmark(4).Name={'Th8'}; % processus spinosus T8
...
CreateBodyJoint('Left_Ankle',4,[2 1],[Z X Y]); % joint 4
CreateBodyJoint('Left_Knee',5,[3 2],[Z X Y]); % joint 5
CreateBodyJoint('Left_Hip',6,[4 3],[Z X Y]); % joint 6
CreateBodyJoint('Pelvis_Trunk',7,[5 4],[Z X Y]); % joint 7
...
AnatomicalCalculationFunction='AnatCalc_LowEx_8Segm.m';
BODY.CONTEXT.AnatomicalCalculationFunction='AnatCalc_LowEx_8Segm';

%=======================================================================
% END ### ProjectModel_LowEx_8Segm ###
