...
for i = 1 : length(trToUseDB.subjects)  % cycle for every subject 
    ...
    for j = 1 : length(trToUseDB.subjects(i).sessions) % cycle for every session
        ...
        for k = 1 : length(trToUseDB.subjects(i).sessions(j).trials) % cycle for every trial
            ...
            if ~forceDataOverwriting
                if ~neverProc            
                    if ~strcmp(protInfo.calPrefix,subject.sessions(j).trials(k).protocolData.calPrefix)
                        datapath = recoverPath(recoveryPath, 'c3d', 'tree', 'session', subject.name, subject.sessions(j).name, []);
                        calib = ProbeAnatomy_AUTO(datapath, protInfo.calPrefix);
                        sto = toc(sta);
                        fprintf('\n     Tr: Anatomy completely probed using recovery path (%s sec)...', num2str(sto));
                    else
                        if subject.sessions(j).trials(k).version > 1
                            ...
                            calib = ProbeAnatomy_AUTO(calibFilesData, protInfo.calPrefix);
                            sto = toc(sta);
                            fprintf('\n     Tr: Anatomy completely probed (%s sec)...', num2str(sto));
                        else
    ...
    %                     fprintf('\n     Tr: Data for dynamic file %s internally recovered ...', datafile);
    %                 else
    %                     [structPath,dummy1,dummy2] = fileparts(subject.sessions(j).trials(k).subPath);
    ...
    %                     fprintf('\n     Tr: Data for dynamic file %s loaded (from file)...', datafile);
    %                 end
    %             else
    %                 datapath = recoverPath(recoveryPath, 'c3d', 'tree', 'session', subject.name, subject.sessions(j).name, []);
    %                 [dummy, ParameterGroup] = BMimportULC3D_3_AUTO(datafile,datapath);  
    %                 fprintf('\n     Tr: Data for dynamic file %s loaded (from file, recovery path used)...', datafile);
    %             end
                % Retrieve raw data
                MARKER_DATA = getMarkersDataFromStruct(subject.sessions(j).trials(k).data.points,BODY.CONTEXT.MarkerLabels);
                MARKER_TIME_OFFSET = 0.;
                ...
                % Trial: Update stick figure
    %                         sta = tic;
    %                         CalculateDefaultStickFigure_NO_WB;
    %                         sto = toc(sta);
    %                         fprintf('\n     Tr: Stick figure updated (for visualization only) (%s sec)...', num2str(sto));
                ...
                sta = tic;
                CalculateJointKinematics_OPT3_ADV('AnatomyBased', []);
                iJointsAbs = [];
                if protInfo.absAngRefPos == 1
                    ...
                    if ~isempty(iJointsAbs)
                        CalculateJointKinematics_OPT3_ADV('ReferenceBased', iJointsAbs);
                    end
                end
                sto = toc(sta);
    ...
    %             if ~forceDataOverwriting
    %                 if ~neverProc
    %                     % Internally recover the events
    %                     eventSide = subject.sessions(j).trials(k).data.stParam.eventsRaw.eventSide;
    %                     eventType = subject.sessions(j).trials(k).data.stParam.eventsRaw.eventType;
    %                     eventTime = subject.sessions(j).trials(k).data.stParam.eventsRaw.eventTime;
    %                     kineFreq = subject.sessions(j).trials(k).data.kineFreq;
    %                 else
    ...
    %             fprintf('\n     Tr: Raw events imported ...');
                ...
                % Trial: Add acquisition frequency information
    %             subject.sessions(j).trials(k).data.kineFreq = kineFreq;
    %             fprintf('\n     Tr: Cameras acquisition frequency imported  ...');
            else
...
% NOTE:
% There are differences in names between this GUI and UpLife - LoLife GUIs.
% UZPell -> UpLife, LoLife:
% stParams -> timeParams
% angles -> angles
% points -> pointsData
% kineFreq -> kineFreq
